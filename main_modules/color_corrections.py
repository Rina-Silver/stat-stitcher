...
def color_transfer(source, target, clip=True, preserve_paper=True):
    ...
    # маска для вычитания из результирующего изображения
    mask = cv2.cvtColor(target.copy().astype(np.uint8), cv2.COLOR_BGR2GRAY)
    mask = (mask > 0) * 255
    ...
    # применяем маску
    resultMasked = cv2.bitwise_and(transfer, transfer, mask=mask)
